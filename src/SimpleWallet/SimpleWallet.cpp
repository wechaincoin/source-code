/*
Copyright (C) 2018, The prospere developers
         ...
         Config &config)
{
    ...
    {
        std::cout << WarningMsg("It looks like prospered isn't open!")
                  << std::endl << std::endl
                  << WarningMsg("Ensure prospered is open and has finished "
                                "initializing.")
                  << std::endl
                  << WarningMsg("If it's still not working, try restarting "
                                "prospered. The daemon sometimes gets stuck.") 
                  << std::endl
                  << WarningMsg("Alternatively, perhaps prospered can't "
        ...
        else if (address.substr(0, 4) != "scsx")
        {
            std::cout << WarningMsg("Invalid address! It should start with "
                                    "\"scsx\"!") << std::endl;
                                                        ...
                                                        address))
        {
            std::cout << WarningMsg("Failed to parse  address! Ensure you "
                           ...
                           CryptoNote::INode &node)
{
    ...
    /* Couldn't get timestamp, maybe old node or prospered closed */
    if (blockTime != "")
    {
                         ...
                         std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    if (localHeight != remoteHeight)
    {
        std::cout << "Your prospered isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting prospered.\n"
...
void connectingMsg()
{
    std::cout << std::endl << "Making initial contact with prospered."
              << std::endl
